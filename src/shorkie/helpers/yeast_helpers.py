"""Sequence / coverage / gradient / plotting helpers for Shorkie analyses.

Consolidated single copy of the former ``yeast_helpers.py`` (9 copies, 4 distinct
versions). Canonical source: the superset from
``analysis/shorkie/track_prediction_eval/3_viz_rnaseq_tracks/yeast_helpers.py``.
``process_sequence`` accepts an optional ``model_type`` ('self_supervised' |
'supervised' | any 'LM' variant | None); None/self-supervised add the species
channel (col 114 = S. cerevisiae), 'supervised' uses the raw 4-channel one-hot.
"""
import os
...
def process_sequence(fasta_open, chrom, start, end, gene_pr, seq_len=16384, model_type='self_supervised'):
    ...
    if model_type is None or model_type == 'self_supervised' or (isinstance(model_type, str) and 'LM' in model_type):
