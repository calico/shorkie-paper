...
# List of negative sets and models to analyze
negsets = [1, 2, 3, 4]
...
for model in models:
    ...
    for negset in negsets:
        ...
        if not os.path.exists(pos_file):
            print(f"  Missing positive file: {pos_file}")
            continue
        if not os.path.exists(neg_file):
            print(f"  Missing negative file: {neg_file}")
            continue
        ...
        try:
            ...
            print(f"  Error reading files for negset {negset}: {e}")
            continue
        ...
        if len(np.unique(y_true)) < 2:
            print(f"  Skipping negset {negset} - only one class present.")
            continue
        ...
        plt.title(f"ROC Curve ({model} - Negative Set {negset})")
        ...
        plt.title(f"Precision-Recall Curve ({model} - Negative Set {negset})")
        ...
        for fold in folds:
            ...
            if not subset.empty:
                ...
                plt.title(f'Positive TSS Distance')
            ...
            if not subset.empty:
                ...
                plt.title(f'Negative TSS Distance')
                plt.xlabel("TSS Distance")
                plt.ylabel("Count")
                plt.tight_layout()
        ...
        try:
            ...
            plt.suptitle(f'Score by TSS Distance Bin and Fold ({model} - Negative Set {negset})')
            plt.savefig(os.path.join(violin_dir, f'{score}_violin_by_fold_negative_set{negset}.png'), dpi=300)
            plt.close()
        except Exception:
            # Violin plot can fail if bins are empty or standard deviation is 0
            plt.close()
        ...
        for i, fold in enumerate(folds):
            ...
            if fold_df.empty or fold_df['label'].nunique() < 2:
                continue
            fold_fpr, fold_tpr, _ = roc_curve(fold_df['label'], np.abs(fold_df[score]))
            fold_roc_auc = auc(fold_fpr, fold_tpr)
            plt.plot(fold_fpr, fold_tpr, lw=1, alpha=0.7, color=fold_colors[i%len(fold_colors)],
                     label=f'Fold {fold} (AUC = {fold_roc_auc:.2f})')
        plt.plot([0, 1], [0, 1], 'r--', lw=1)
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
        plt.title(f'ROC by Fold ({model} - Negative Set {negset})')
        ...
        for i, fold in enumerate(folds):
            ...
            plt.plot(fold_recall, fold_precision, lw=1, alpha=0.7, color=fold_colors[i%len(fold_colors)],
                     label=f'Fold {fold} (AP = {fold_pr_auc:.2f})')
        plt.xlabel('Recall')
        plt.ylabel('Precision')
        plt.title(f'Precision-Recall by Fold ({model} - Negative Set {negset})')
        plt.ylim(-0.05, 1.1)
        ...
        for tss_group in labels:
            ...
            plt.xlabel("False Positive Rate")
            plt.ylabel("True Positive Rate")
            plt.title(f"ROC - Group {tss_group} ({model} - NegSet {negset})")
            plt.legend(loc="lower right")
            plt.tight_layout()
            ...
            plt.xlabel("Recall")
            plt.ylabel("Precision")
            plt.title(f"PR - Group {tss_group} ({model} - NegSet {negset})")
            plt.legend(loc="lower left")
            plt.tight_layout()
        ...
        if tss_auroc and tss_auprc:
            ...
            plt.title(f"Performance vs. TSS Group ({model} - Negative Set {negset})")
            plt.ylim(0, 1)
            plt.legend()
            plt.tight_layout()
    ...
    # -------------------------
    # Combined Trend Plot Across All Negative Sets (Per Model)
    # -------------------------
    # Determine ordered TSS groups that have data across any negative set.
    all_groups_set = set()
    for neg in trend_auroc_all:
        all_groups_set.update(trend_auroc_all[neg].keys())
    
    if not all_groups_set:
        print(f"No TSS group data for model {model}. Skipping trend plot.")
        continue

    ordered_groups = [g for g in labels if g in all_groups_set]
    x_idx = np.arange(len(ordered_groups))
    ...
    # For AUROC: scatter each negative set's dot
    for i, neg in enumerate(negsets):
        ...
        axes[0].scatter(xs, ys, color=neg_colors[i], label=f"Set {neg}")

    # Fit a linear regression to the ensemble average AUROC
    mask = ~np.isnan(mean_auroc)
    if np.sum(mask) > 1:
        coeffs_auroc = np.polyfit(x_idx[mask], np.array(mean_auroc)[mask], 1)
        trend_line_auroc = np.polyval(coeffs_auroc, x_idx)
        axes[0].plot(x_idx, trend_line_auroc, color='k', linewidth=2, label="Ensemble Trend")
        
        # Fill between
        axes[0].fill_between(x_idx, 
                             np.array(mean_auroc) - np.nan_to_num(se_auroc),
                             np.array(mean_auroc) + np.nan_to_num(se_auroc), 
                             color='k', alpha=0.3)
    
    axes[0].set_ylabel("AUROC")
    axes[0].set_title(f"AUROC vs. TSS Group ({model} - Ensemble)")
    ...
    for i, neg in enumerate(negsets):
        ...
        axes[1].scatter(xs, ys, color=neg_colors[i], label=f"Set {neg}")

    # Fit a linear regression to the ensemble average AUPRC
    mask = ~np.isnan(mean_auprc)
    if np.sum(mask) > 1:
        coeffs_auprc = np.polyfit(x_idx[mask], np.array(mean_auprc)[mask], 1)
        trend_line_auprc = np.polyval(coeffs_auprc, x_idx)
        axes[1].plot(x_idx, trend_line_auprc, color='k', linewidth=2, label="Ensemble Trend")
        axes[1].fill_between(x_idx, 
                             np.array(mean_auprc) - np.nan_to_num(se_auprc),
                             np.array(mean_auprc) + np.nan_to_num(se_auprc), 
                             color='k', alpha=0.3)

    axes[1].set_xlabel("TSS Group")
    axes[1].set_ylabel("AUPRC")
    axes[1].set_title(f"AUPRC vs. TSS Group ({model} - Ensemble)")
    ...
    # -------------------------
    # Ensemble Overall ROC/PR Plots Across Negative Sets (Per Model)
    # -------------------------
    if ensemble_roc_curves:
        ...
        for i, roc_data in enumerate(ensemble_roc_curves):
            ...
            plt.plot(roc_data['fpr'], roc_data['tpr'], color=ens_colors[i%len(ens_colors)], lw=1, alpha=0.7,
                     label=f"NegSet {roc_data['negset']} (AUC={roc_data['auc']:.2f})")
        
        if all_tpr_interp:
            ensemble_tpr = np.mean(all_tpr_interp, axis=0)
            ensemble_auc = auc(fpr_grid, ensemble_tpr)
            plt.plot(fpr_grid, ensemble_tpr, color='k', lw=2.5, label=f"Average (AUC={ensemble_auc:.2f})")
        
        plt.plot([0, 1], [0, 1], 'r--', lw=1)
        plt.xlabel("False Positive Rate")
        plt.ylabel("True Positive Rate")
        plt.title(f"Ensemble ROC Curve ({model})")
        plt.legend(loc="lower right", fontsize=8)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "ensemble_roc_curve.png"), dpi=300)
        plt.close()

    if ensemble_pr_curves:
        ...
        for i, pr_data in enumerate(ensemble_pr_curves):
            # PR curve interp logic
            interp_precision = np.interp(recall_grid, pr_data['recall'][::-1], pr_data['precision'][::-1])
            all_precision_interp.append(interp_precision)
            plt.plot(pr_data['recall'], pr_data['precision'], color=ens_colors[i%len(ens_colors)], lw=1, alpha=0.7,
                     label=f"NegSet {pr_data['negset']} (AP={pr_data['ap']:.2f})")
        
        if all_precision_interp:
            ensemble_precision = np.mean(all_precision_interp, axis=0)
            ensemble_ap = auc(recall_grid, ensemble_precision)
            plt.plot(recall_grid, ensemble_precision, color='k', lw=2.5, label=f"Average (AP={ensemble_ap:.2f})")
        
        plt.xlabel("Recall")
        plt.ylabel("Precision")
        plt.title(f"Ensemble PR Curve ({model})")
        plt.ylim(-0.05, 1.1)
        plt.legend(loc="lower left", fontsize=8)
        plt.tight_layout()
