...
parser = argparse.ArgumentParser(description="Extract positive sequences around eQTLs.")
parser.add_argument("--tsv_file", required=True, help="Input eQTL TSV/CSV file")
parser.add_argument("--fasta_file", required=True, help="Reference FASTA file")
parser.add_argument("--gtf_file", required=True, help="Reference GTF file")
parser.add_argument("--output_tsv", default="results/output_pos_sequences.tsv", help="Output file")
parser.add_argument("--dataset", choices=["renganaath", "kita", "caudal"], default="renganaath", help="Dataset format")
...
####################################
eqtls = pd.read_csv(tsv_file, sep="," if tsv_file.endswith(".csv") else "\t")
...
for idx, row in eqtls.iterrows():
    ...
    if args.dataset == "renganaath":
        raw_chrom = row["chr"]
        try:
            eqtl_pos = int(row["pos"])
        except ValueError:
            print(f"Invalid pos at row {idx}. Skipping.", file=sys.stderr)
            continue
        ref_allele = str(row["ref"]).strip()
        alt_field = str(row["alt"]).strip()
        gene = row["gene"]
        raw_gene_chr = raw_chrom
    else:
        # Default for kita/caudal (assuming standard columns)
        raw_chrom = row.get("Chr", row.get("chr"))
        try:
            eqtl_pos = int(row.get("ChrPos", row.get("pos")))
        except ValueError:
            print(f"Invalid pos at row {idx}. Skipping.", file=sys.stderr)
            continue
        ref_allele = str(row.get("Reference", row.get("ref"))).strip()
        alt_field = str(row.get("Alternate", row.get("alt"))).strip()
        gene = row.get("Pheno", row.get("gene"))
        raw_gene_chr = row.get("Pheno_chr", raw_chrom)
