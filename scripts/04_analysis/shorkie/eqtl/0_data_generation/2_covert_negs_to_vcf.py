...
def process_file(input_file, output_dir):
    ...
    positive_vcf = os.path.join(output_dir, f"{base}.positive.vcf")
    negative_vcf = os.path.join(output_dir, f"{base}.negative.vcf")
...
def main():
    parser = argparse.ArgumentParser(description="Convert negative eQTL TSVs to VCFs.")
    parser.add_argument("--negative_tsv_prefix", required=True, help="Prefix path for negative TSVs (e.g. results/negative_eqtls_set)")
    parser.add_argument("--output_dir", default="results", help="Directory to save output VCFs")
    parser.add_argument("--iterations", type=int, default=4, help="Number of negative sets to process")
    ...
    for i in range(1, args.iterations + 1):
        input_files.append(f"{args.negative_tsv_prefix}{i}.tsv")
