...
def main():
    parser = argparse.ArgumentParser(description="Process GWAS data to identify local/distant eQTLs.")
    parser.add_argument("--gwas_input", required=True, help="Input GWAS tabular file")
    parser.add_argument("--output_dir", required=True, help="Directory to save output files")
    args = parser.parse_args()

    gwas_input = args.gwas_input
    output_dir = args.output_dir
    os.makedirs(output_dir, exist_ok=True)
    
    input_basename = os.path.basename(gwas_input).replace('.tab', '').replace('.tsv', '').replace('.csv', '')
    gwas_output = os.path.join(output_dir, f'{input_basename}_cleaned.tab')
    gwas_CIS_output = os.path.join(output_dir, f'{input_basename}_cleaned_CIS.tab')
    gwas_TRANS_output = os.path.join(output_dir, f'{input_basename}_cleaned_TRANS.tab')

    # Load the GWAS data
    gwas_df = pd.read_csv(gwas_input, sep='\t')
...
snv_data_CIS = snv_data[snv_data['is_local'] == True]
snv_data_TRANS = snv_data[snv_data['is_local'] == False]
