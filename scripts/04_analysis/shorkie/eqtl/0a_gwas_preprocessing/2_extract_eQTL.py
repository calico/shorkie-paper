...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Intersect GWAS eQTLs with a GVCF background.")
    parser.add_argument("--gvcf_file", required=True, help="Input GVCF file")
    parser.add_argument("--gwas_input_prefix", required=True, help="Prefix for GWAS input files (e.g. data_CIS.tab)")
    parser.add_argument("--output_dir", required=True, help="Output directory for VCFs")
    ...
    for distance_type in ["CIS", "TRANS"]:  
        # Define file paths
        eqtl_file = f"{args.gwas_input_prefix}_{distance_type}.tab"
    ...
    only_eqtl_df.to_csv(only_eqtl_tsv_file, sep='\t', index=False)
        ...
        only_eqtl_vcf_file = os.path.join(output_dir, f"only_eqtl_data_{distance_type}.vcf")
