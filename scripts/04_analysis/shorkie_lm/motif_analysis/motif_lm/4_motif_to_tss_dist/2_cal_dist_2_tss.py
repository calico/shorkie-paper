...
if __name__ == "__main__":
    ...
    motif_bed = "tf_modisco_motif_hits.bed"  # Your motif BED file.
    tss_file = "tss.bed"                     # Your TSS file.
    genome_file = "genome.chrom.sizes"       # A two-column file: chromosome and size.
    
    # Calculate distances for motif hits.
    df_motif = calculate_closest_tss_distance(motif_bed, tss_file)
    df_motif.to_csv("motif_tss_distances.csv", index=False)
    print("Saved motif TSS distances to motif_tss_distances.csv")
    
    # Calculate distances for background (random) positions.
    num_background = 1  # Change this number as desired.
    df_background = calculate_background_tss_distance(motif_bed, tss_file, genome_file, num_background=num_background)
    df_background.to_csv("background_tss_distances.csv", index=False)
    print("Saved background TSS distances to background_tss_distances.csv")
    
    # Display a sample of each.
    print("\nSample motif results:")
    print(df_motif.head())
    
    print("\nSample background results:")
