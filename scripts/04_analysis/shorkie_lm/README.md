...
| `lm_SMT3_viz/` | SMT3 case study: `1`–`2` DNA-logo PWMs (SpeciesLM vs Shorkie_LM), `3_inference_smt3_unmasked` (run the LM over SMT3 windows), `4_viz_smt3_logo_unmasked` (logo of predictions), `5_verify_alignment` (coordinate check). Nested `dependency_map/` (own README) computes nucleotide dependency maps. |
...
| `motif_lm__unseen_species/` | Cross-species generalization on held-out clades — **see its own [README](motif_analysis/motif_lm__unseen_species/README.md)**. |
...
> (not the `yeast_ml` TF env) and adapts a third-party method — see its
> [README](lm_SMT3_viz/dependency_map/README.md). Generated outputs are not
> committed; run via `sbatch <step>.sh`, or adapt through `scripts/common/submit.sh`.
