...
###########################################################################
# Main routine: plot two logos per sequence for T90 vs. T0 differences        #
###########################################################################
def main():
    ...
    # Get the track indices for T0 and T90
    target_df_t0 = target_df[target_df['timepoint'] == 'T0']
    target_df_t90 = target_df[target_df['timepoint'] == 'T90']
    if target_df_t0.empty or target_df_t90.empty:
        raise ValueError("Target file does not contain both T0 and T90 timepoints.")
    ...
    selected_track_indices_t90 = target_df_t90['index'].astype(int).tolist()
    adjusted_track_indices_t90 = [idx - track_offset for idx in selected_track_indices_t90]

    print("T0 raw track indices:", selected_track_indices_t0)
    print("T0 adjusted track indices:", adjusted_track_indices_t0)
    print("T90 raw track indices:", selected_track_indices_t90)
    print("T90 adjusted track indices:", adjusted_track_indices_t90)
    ...
    with h5py.File(f"{options.exp_dir}/scores.h5", 'r') as h5_file:
        ...
        for score in scores:
            ...
            diff_dir_norm = f"{options.exp_dir}/dna_logo_example/{options.target_tf}logos/{score}/T0_vs_T90_diff_normalized"
            diff_dir_raw = f"{options.exp_dir}/dna_logo_example/{options.target_tf}logos/{score}/T0_vs_T90_diff_raw"
            ...
            for seq_idx in range(N_sequences):
                ...
                # Compute PWM averages for T0 and T90
                pwm_t0 = compute_time_average_for_one_time(dataset, seq_idx, track_indices=adjusted_track_indices_t0)
                pwm_t90 = compute_time_average_for_one_time(dataset, seq_idx, track_indices=adjusted_track_indices_t90)
                ...
                mean_t90 = np.mean(pwm_t90, axis=-1, keepdims=True)
                pwm_norm_t90 = pwm_t90 - np.tile(mean_t90, (1, 4))
                # Compute the normalized difference (T90 minus T0)
                diff_norm = pwm_norm_t90 - pwm_norm_t0
