...
current_experiment="${WORK_ROOT}/seq_experiment/${fine_tuned_model_type}/16bp/self_supervised_unet_small_bert_drop/train/${fold_param}/train"
...
bed_file="${WORK_ROOT}/data/gene_exp_ism_window/${exp_data}_chunk/${exp_data}_windows_$(printf "%03d" $SLURM_ARRAY_TASK_ID).bed"
if [ -f "$bed_file" ]; then
    # Read the first three columns: chrom, start, and end.
    read chrom start end rest < "$bed_file"
    length=$(( end - start + 1 ))
...
python ${BASKERVILLE_SCRIPTS}/hound_ism_bed.py \
    ...
    ${WORK_ROOT}/seq_experiment/${fine_tuned_model_type}/16bp/self_supervised_unet_small_bert_drop/train/${fold_param}/train/params.json \
    ${WORK_ROOT}/seq_experiment/${fine_tuned_model_type}/16bp/self_supervised_unet_small_bert_drop/train/${fold_param}/train/model_best.h5 \
    "$bed_file" \
