...
###########################################################################
# Main routine: plot two logos per sequence for T180 vs. T0 differences        #
###########################################################################
def main():
    ...
    # Get the track indices for T0 and T180
    target_df_t0 = target_df[target_df['timepoint'] == 'T0']
    target_df_t180 = target_df[target_df['timepoint'] == 'T180']
    if target_df_t0.empty or target_df_t180.empty:
        raise ValueError("Target file does not contain both T0 and T180 timepoints.")
    ...
    selected_track_indices_t180 = target_df_t180['index'].astype(int).tolist()
    adjusted_track_indices_t180 = [idx - track_offset for idx in selected_track_indices_t180]

    print("T0 raw track indices:", selected_track_indices_t0)
    print("T0 adjusted track indices:", adjusted_track_indices_t0)
    print("T180 raw track indices:", selected_track_indices_t180)
    print("T180 adjusted track indices:", adjusted_track_indices_t180)
    ...
    with h5py.File(f"{options.exp_dir}/scores.h5", 'r') as h5_file:
        ...
        for score in scores:
            ...
            diff_dir_norm = f"{options.exp_dir}/dna_logo_example/{options.target_tf}logos/{score}/T0_vs_T180_diff_normalized"
            diff_dir_raw = f"{options.exp_dir}/dna_logo_example/{options.target_tf}logos/{score}/T0_vs_T180_diff_raw"
            ...
            for seq_idx in range(N_sequences):
                ...
                # Compute PWM averages for T0 and T180
                pwm_t0 = compute_time_average_for_one_time(dataset, seq_idx, track_indices=adjusted_track_indices_t0)
                pwm_t180 = compute_time_average_for_one_time(dataset, seq_idx, track_indices=adjusted_track_indices_t180)
                ...
                mean_t180 = np.mean(pwm_t180, axis=-1, keepdims=True)
                pwm_norm_t180 = pwm_t180 - np.tile(mean_t180, (1, 4))
                # Compute the normalized difference (T180 minus T0)
                diff_norm = pwm_norm_t180 - pwm_norm_t0
