...
def main():
    ...
    for score_file in score_files:
        with h5py.File(score_file, 'r') as h5_file:
            ...
            # These do not depend on time point; they will be used for every group.
    ...
    ##############################################
    # Process and save the final NPZ files per time point
    ##############################################
    for score in scores:
        ...
        for time in group2h5_indices:
            # Concatenate collected scores and references along the sequence axis.
            pred_scores_group = np.concatenate(collected_scores[score][time], axis=0)
            ref_scores_group = np.concatenate(collected_ref_scores[score][time], axis=0)
            # pred_scores_group has shape: (N_total, L, 4, num_tracks_group)
            # ref_scores_group has shape: (N_total, L, 4)
            num_tracks_group = pred_scores_group.shape[-1]
            print(f"For score '{score}' and time '{time}', pred shape: {pred_scores_group.shape} and ref shape: {ref_scores_group.shape}")
            ...
            # Transpose to (N_total, 4, L)
            ref_arr_trans = np.transpose(ref_scores_group, (0, 2, 1))
            # Tile the reference array along the first dimension (num_tracks_group times)
            ref_tiled = np.tile(ref_arr_trans, (num_tracks_group, 1, 1))
            ref_out_npz = os.path.join(out_dir_time, "ref.npz")
            np.savez(ref_out_npz, arr_0=ref_tiled)
            print(f"Saved ref.npz with shape {ref_tiled.shape} in {out_dir_time}")

            # Process and save pred_scores.
            # Collect all track arrays after transposing.
            track_list = []
            for track_idx in range(num_tracks_group):
                # Extract track data => shape: (N_total, L, 4)
                track_data = pred_scores_group[..., track_idx]
                # Transpose => shape: (N_total, 4, L)
                track_trans = np.transpose(track_data, (0, 2, 1))
                track_list.append(track_trans)
            # Concatenate along the sequence axis => shape: (N_total*num_tracks_group, 4, L)
            pred_concat = np.concatenate(track_list, axis=0)
            pred_out_npz = os.path.join(out_dir_time, "pred.npz")
            np.savez(pred_out_npz, arr_0=pred_concat)
            print(f"Saved pred.npz with shape {pred_concat.shape} in {out_dir_time}")
