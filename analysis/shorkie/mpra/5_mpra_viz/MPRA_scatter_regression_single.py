...
# GROUND_TRUTH_EXP = np.log(GROUND_TRUTH_EXP + 1)  # Log-transform the ground truth expressions
# count the number of Nans in the ground truth expressions
nan_count = np.sum(np.isnan(GROUND_TRUTH_EXP)) 
print(f"Number of NaN values in ground truth expressions: {nan_count}")
...
def compute_correlation_for_gene(gene_stats, ground_truth, transform=False):
    ...
    pred = pred[mask]
    if len(gt) < 2:
        # not enough points to correlate
        return np.nan, np.nan, gt, pred
...
def visualize_scatter_by_distance_group(gene_list, ground_truth, seq_type, group_label, result_base_dir, transform=False):
    ...
    for distance in distances:
        ...
        # drop NaNs / infs
        mask = np.isfinite(gt_plot) & np.isfinite(pred_plot)
        gt_f = gt_plot[mask]
        pd_f = pred_plot[mask]
        if len(gt_f) < 2 or np.std(gt_f)==0 or np.std(pd_f)==0:
            corr_pearson, corr_spearman = 0.0, 0.0
        else:
            corr_pearson, _   = pearsonr(gt_f, pd_f)
            corr_spearman, _ = spearmanr(gt_f, pd_f)
...
def plot_trend_correlation_group(gene_list, ground_truth, seq_type, group_label, result_base_dir, transform=False):
    ...
    plt.xlabel("Insertion Distance (bp)", fontsize=12)
...
def analyze_and_plot_group(aggregated_data, ground_truth, seq_type, group_label, result_base_dir, transform=False):
    ...
    if len(aggregated_gt) < 2:
        print(f"Not enough valid points for {group_label} / {seq_type}")
        return
