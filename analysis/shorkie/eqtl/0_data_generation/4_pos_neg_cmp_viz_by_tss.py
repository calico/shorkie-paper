...
def process_fold_data(fold, score, df_eqtl, root_dir):
    """
    ...
    """
    snp_dir = os.path.join(root_dir, f'experiments/eQTL_exp/eqtls/eqtl_{fold}/')
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate positive vs negative eQTL scores by TSS distance.")
    parser.add_argument("--root_dir", required=True, help="Root directory pointing to experiments")
    parser.add_argument("--eqtl_tsv", required=True, help="Positive eQTL TSV file")
    parser.add_argument("--gtf_file", required=True, help="GTF file")
    parser.add_argument("--output_dir", default="./viz", help="Output directory")
    args = parser.parse_args()

    # Set up paths and directories
    root_dir = args.root_dir
    eqtl_tsv = args.eqtl_tsv
    ...
    for fold in folds:
        merged_df, _ = process_fold_data(fold, score, df_eqtl, root_dir)
