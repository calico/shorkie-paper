...
def main():
    parser = argparse.ArgumentParser(description="Generate Manhattan plot from GWAS data.")
    parser.add_argument("--gwas_input", required=True, help="Input GWAS tabular file")
    parser.add_argument("--output_file", default="manhattan_plot.png", help="Output PNG filename")
    args = parser.parse_args()

    # --- 1. Load and filter data ---
    # Load the GWAS data
    gwas_df = pd.read_csv(args.gwas_input, sep='\t')

    if 'ld_mask' in gwas_df.columns:
        gwas_df = gwas_df[gwas_df['ld_mask'] != "masked"]
    print(gwas_df.head())
    print(len(gwas_df))
    # Filter out missing or infinite p-values
    if 'PValue' not in gwas_df.columns:
        print("Warning: 'PValue' column not found in GWAS input.")
        return
