...
def process_predictions(predictor, input_sequences_file, output_file):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing eQTLs"):
        ...
        # Debug print can be commented out or replaced by logging
        # print(f"Processing row {idx}: ref_seq: {ref_seq} ; alt_seq: {alt_seq}")
...
def main():
    ...
    for model in models:
        ...
        input_sequences_file = "results/output_pos_sequences.tsv"  # generated from previous step
        output_file = f"results/{MODEL_NAME}/final_pos_predictions.tsv"
