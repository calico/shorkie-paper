...
if [ "${current_dataset}" = "RP" ]; then
    ...
    eval_dir="${WORK_ROOT}/experiments/motif_LM_RP_TSS/lm_saccharomycetales_gtf_${model}/eval_RP"
...
python 2_modisco_DNA_logo.py --model_arch ${model} \
    --modisco_h5 ${WORK_ROOT}/experiments/motif_LM_RP_TSS/lm_saccharomycetales_gtf_${model}/eval_${current_dataset}/modisco_results_w16384_n100000.h5 \
    --motifs_html ${WORK_ROOT}/experiments/motif_LM_RP_TSS/lm_saccharomycetales_gtf_${model}/eval_${current_dataset}/report_high_conf/motifs.html \
    --out_dir $out_dir/ \
    --seq_bed ${bedfile} \
    --trim_threshold 0.3 \
    --trim_min_length 3 \
