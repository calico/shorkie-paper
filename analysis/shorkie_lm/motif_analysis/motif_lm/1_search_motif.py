...
def main():
    ...
    # # Create a boolean mask where weight_scale equals zero
    # mask = (weight_scale == 0)  # shape: (528, 16384)

    # # Expand and broadcast the mask to have the same last dimension as x_true/x_pred
    # mask_expanded = mask[:, :, np.newaxis]  # (528, 16384, 1)
    # mask_expanded = np.repeat(mask_expanded, 4, axis=2)  # (528, 16384, 4)

    # # Now the mask dimensions match those of x_true and x_pred
    # x_true[mask_expanded] = 0.25
    # x_pred[mask_expanded] = 0.25

    print("x_true: ", x_true.shape)
    print("x_pred: ", x_pred.shape)
    print("Applied mask to x_true and x_pred.")
    ...
    print("x_true: ", x_true.shape)
    print("x_pred: ", x_pred.shape)
    print("x_pred.max(): ", x_pred.max(), '; x_pred.min(): ', x_pred.min())
    print("label: ", label.shape)
