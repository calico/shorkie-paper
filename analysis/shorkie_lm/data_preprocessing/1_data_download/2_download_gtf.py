...
'''
download_reference_gtfs.py

Download gtf files from Ensembl Fungi.
'''
...
def main():
    ...
    os.makedirs(options.out_dir + 'data' + options.save_suffix + '/gtf', exist_ok=True)
    ...
    for _, row in species_df.iterrows():
        ...
        if species_str == core_str :
            url = '\'https://ftp.ebi.ac.uk/ensemblgenomes/pub/release-59/fungi/gtf/' + species_str + '/' + species_str2 + '.' + assembly_str + '.59.gtf.gz\''
        else :
            url = '\'https://ftp.ebi.ac.uk/ensemblgenomes/pub/release-59/fungi/gtf/' + core_str + '/' + species_str + '/' + species_str2 + '.' + assembly_str + '.59.gtf.gz\''
        
        local_path = options.out_dir + 'data' + options.save_suffix + '/gtf/' + accession_id_cleaned + '.59.gtf.gz'
