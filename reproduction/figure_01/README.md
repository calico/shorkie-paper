...
|---|---|---|---|---|---|
| **A** | Shorkie-LM architecture: multi-resolution U-Net (1→128 bp) + 8 transformer blocks, masked-token (MLM) head | schematic | `recheck/build_panels_ABCE.py::build_1A()` (renders the trunk block-stack from the config; no upstream script) | released LM `params.json` → `models.shorkie_lm` | no |
