...
|---|---|---|---|---|---|
| **A** | Shorkie-LM architecture: multi-resolution U-Net (1→128 bp) + 8 transformer blocks, masked-token (MLM) head | schematic | — **skipped** (hand-drawn schematic; not reproduced). Architecture is defined by the released `params.json` | released LM `params.json` → `models.shorkie_lm` | no |
...
| **E** | Data-preprocessing pipeline: genome chunking (16384 bp, 4096 overlap) → one-hot tensors + region loss weights → homolog removal + 7% repeat-fraction threshold → train/valid/test chr split | schematic | — **skipped** (hand-drawn schematic; not reproduced). Pipeline lives in `scripts/01_data_build/lm_corpus/` (`1_generate_sequences_bed.py` + `bed_helper.py`, `write_data*.py`) | one example 16,384 bp window | no |
...
- **1A / 1E** are hand-drawn schematics in the paper (architecture diagram; preprocessing pipeline) — **skipped**, not reproduced. The architecture is defined by the released `params.json`; the preprocessing pipeline lives in `scripts/01_data_build/lm_corpus/`.

### Discrepancy log
| Panel | Discrepancy | Cause | Impact |
|---|---|---|---|
| 1A, 1E | **skipped** — not reproduced | panels are hand-drawn schematics (no data figure to regenerate) | none (consistent with how the other figures treat schematic panels) |
...
**Verdict: Figure 1 data panels fully reproduced.** Computational panels (F, G) numerically exact (12/12, Δ=0); heavy-external panels (B, C, D) recomputed from genomes (byte-identical on re-run) with the published trends reproduced; schematics (A, E) are **skipped** (hand-drawn, not reproduced).
...
- **Exact-render pass (D/F/G):** panels D, F, G re-rendered to match the published crops in style **and**
  ...
  the plain released bar chart); 1F/1G widened to the published wide aspect. B/C left as-is; A/E removed (schematics).
