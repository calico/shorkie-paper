...
- **Reproduce:** [`fig07_eqtl_variant_effects.ipynb`](../../notebooks/fig07_eqtl_variant_effects.ipynb) (executed, 0 errors).
...
**Changes to legacy scripts:** none. `1_roc_pr_shorkie_fold.py` / `2_AUROC_AUPRC_by_dsitance.py` were ported into notebook cells with `args.root_dir` → `shorkie.config` (identical metric/label/binning logic, verified by the Δ=0.000 exact match). The GPU ISM reuses `shorkie.models.ensemble` (cf. `panels/run_ism_eqtl.py`).
