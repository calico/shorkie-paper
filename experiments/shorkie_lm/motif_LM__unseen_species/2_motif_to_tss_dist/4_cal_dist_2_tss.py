...
if __name__ == "__main__":
    # Files (update these file paths as needed)
    ...
    motif_tss_distances_file = f"{root_dir}/motif_tss_distances.csv"
    bg_file = f"{root_dir}/background_tss_distances.csv"
    
    # Calculate motif distances.
    df_motif = calculate_closest_tss_distance(motif_bed, tss_file)
    df_motif.to_csv(motif_tss_distances_file, index=False)
    print("Saved motif TSS distances to motif_tss_distances.csv")
    
    # Calculate background distances.
    # Here num_background specifies how many random positions per motif.
    ...
    df_background.to_csv(bg_file, index=False)
