...
# # 12) Bar chart of log₂ fold-change vs. T0 for measured vs. predicted
    # # --------------------------------------------------------------------
    # # identify the T0 baseline track
    # t0_ids = acc_df.loc[acc_df['timepoint'] == 0, 'identifier'].tolist()
    # if not t0_ids:
    #     print("No T0 baseline track found; skipping log₂ FC bar chart.")
    # else:
    #     baseline_id = t0_ids[0]
    #     # compute median ± SEM of log₂ FC for each timepoint ≠ 0
    #     fc_tps = [t for t in sorted(acc_df['timepoint'].dropna().unique()) if t != 0]
    #     med_fc_meas, se_fc_meas = [], []
    #     med_fc_pred, se_fc_pred = [], []

    #     for t in fc_tps:
    #         ids_t = acc_df.loc[acc_df['timepoint'] == t, 'identifier'].tolist()
    #         if not ids_t:
    #             continue
    #         # Δ = log₂(x+1)_t − log₂(x+1)_T0
    #         delta_meas = (gene_targets[ids_t]
    #                       .subtract(gene_targets[baseline_id], axis=0)
    #                       .values.flatten())
    #         delta_pred = (gene_preds[ids_t]
    #                       .subtract(gene_preds[baseline_id], axis=0)
    #                       .values.flatten())
    #         # filter out any non-finite values
    #         delta_meas = delta_meas[np.isfinite(delta_meas)]
    #         delta_pred = delta_pred[np.isfinite(delta_pred)]

    #         med_fc_meas.append(np.median(delta_meas))
    #         se_fc_meas.append(sem(delta_meas))
    #         med_fc_pred.append(np.median(delta_pred))
    #         se_fc_pred.append(sem(delta_pred))

    #     # plot side-by-side bar chart
    #     x = np.arange(len(fc_tps))
    #     w = 0.35
    #     plt.figure(); ax = plt.gca()
    #     plt.bar(x - w/2, med_fc_meas, w, yerr=se_fc_meas, capsize=5,
    #             label='Measured log₂ FC')
    #     plt.bar(x + w/2, med_fc_pred, w, yerr=se_fc_pred, capsize=5,
    #             label='Predicted log₂ FC')
    #     plt.xticks(x, fc_tps)
    #     plt.xlabel('Timepoint')
    #     plt.ylabel('Median log₂ fold-change vs T0')
    #     plt.title(f'Log₂ fold-change vs T0 for {args.tf}')
    #     plt.legend()
    #     plt.tight_layout()

    #     fp_fcbar = os.path.join(out_dir, 'log2_fc_vs_T0_bar.png')
    #     plt.savefig(fp_fcbar, dpi=300)
    #     plt.close()
    #     print(f"→ saved log₂ FC bar chart: {fp_fcbar}")


    # # 12) Bar chart of raw fold-change relative to T0
    # # -----------------------------------------------------------------------
    # # find T0 baseline track
    # t0_ids = acc_df.loc[acc_df['timepoint'] == 0, 'identifier'].tolist()
    # if not t0_ids:
    #     print("No T0 baseline track found; skipping fold-change bar chart.")
    # else:
    #     baseline_id = t0_ids[0]
    #     # only non-zero timepoints
    #     fc_tps = [t for t in tps if t != 0]
    #     med_fc_t, se_fc_t = [], []
    #     med_fc_p, se_fc_p = [], []
    ...
    #         ids_t = acc_df.loc[acc_df['timepoint'] == t, 'identifier'].tolist()
    #         if not ids_t:
    #             # skip if no track for this timepoint
    #             continue
    #         tid = ids_t[0]

    #         # per-gene fold-change = value_at_t / value_at_T0
    #         fc_meas = raw_t[tid] / raw_t[baseline_id]
    #         fc_pred = raw_p[tid] / raw_p[baseline_id]

    #         # collect median and SEM (omit NaNs)
    #         med_fc_t.append(np.nanmedian(fc_meas))
    #         se_fc_t.append(sem(fc_meas, nan_policy='omit'))
    #         med_fc_p.append(np.nanmedian(fc_pred))
    #         se_fc_p.append(sem(fc_pred, nan_policy='omit'))

    #     # plot
    #     x2 = np.arange(len(med_fc_t))
    #     w2 = 0.35
    #     plt.figure(); ax2 = plt.gca()
    #     plt.bar(x2 - w2/2, med_fc_t, w2, yerr=se_fc_t, capsize=5, label='Measured FC')
    #     plt.bar(x2 + w2/2, med_fc_p, w2, yerr=se_fc_p, capsize=5, label='Predicted FC')
    #     plt.xticks(x2, fc_tps)
    #     plt.xlabel('Timepoint')
    #     plt.ylabel('Fold-change (relative to T0)')
    #     subtitle = f'gene {gene_name}' if gene_name else args.tf
    #     plt.title(f'Raw Fold-change by timepoint (vs T0) for {subtitle}')
    #     plt.legend()
    #     plt.tight_layout()

    #     fp_fc = os.path.join(out_dir, 'raw_fold_change_by_timepoint_bar.png')
    #     plt.savefig(fp_fc, dpi=300)
    #     plt.close()
    #     print(f"→ saved raw fold-change bar chart: {fp_fc}")



    # # 12) Scatter‐plots of log‐fold‐change (Tt − T0) for measured vs. predicted
    # # -----------------------------------------------------------------------
    # t0_ids = acc_df.loc[acc_df['timepoint'] == 0, 'identifier'].tolist()
    # if not t0_ids:
    #     print("No T0 baseline track found; skipping fold‐change scatter plots.")
    # else:
    #     baseline_id = t0_ids[0]
    #     for t in sorted(acc_df['timepoint'].dropna().unique()):
    #         if t == 0:
    ...
    #             continue
    #         tid = ids_t[0]
    #         delta_meas = gene_targets[tid] - gene_targets[baseline_id]
    #         delta_pred = gene_preds[tid]   - gene_preds[baseline_id]

    #         print("delta_meas: ", delta_meas.shape)
    #         print("delta_pred: ", delta_pred.shape)


    #         plt.figure(figsize=(6,6))
    #         plt.scatter(delta_meas, delta_pred, s=10, alpha=0.6)
    #         lims = [
    #             min(delta_meas.min(), delta_pred.min()),
    #             max(delta_meas.max(), delta_pred.max())
    #         ]
    #         plt.plot(lims, lims, '--', linewidth=1)
    #         plt.xlim(lims)
    #         plt.ylim(lims)
    #         plt.xlabel(f'Measured log₂ FC (T{t} − T0)')
    #         plt.ylabel(f'Predicted log₂ FC (T{t} − T0)')
    #         plt.title(f'Fold‐change scatter: T{t} vs T0 for {args.tf}')
    #         plt.tight_layout()

    #         fp_scatter = os.path.join(out_dir, f'fold_change_T{t}_scatter.png')
    #         plt.savefig(fp_scatter, dpi=300)
    #         plt.close()
    #         print(f"→ saved fold‐change scatter plot: {fp_scatter}")
