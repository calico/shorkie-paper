...
def load_eqtl_data(eqtl_tsv):
    ...
    Expects columns: Chr, ChrPos, Pheno, effectSize, Reference, Alternate.
    """
    try:
    ...
    print(f"Loaded {len(df_eqtl)} eQTLs from {eqtl_tsv}")
...
def process_fold_data(fold, score, df_eqtl):
    ...
    """
    snp_dir = f'/home/kchao10/scr4_ssalzbe1/khchao/Yeast_ML/experiments/SUM_data_process/eQTL_kita_etal_select_exp/eqtls/eqtl_{fold}/'
...
def main():
    ...
    ##########################################################
    pos_master = df_eqtl[['Position_Gene', 'effectSize']].copy()
    ...
    pos_long = pd.melt(pos_master, 
                       id_vars=['Position_Gene','effectSize'], 
    ...
    combined_df = pd.concat([pos_df[['Position_Gene','effectSize','score','distance','fold','distance_bin','label','label_type']],
    ...
    for fold in sorted(combined_df['fold'].unique()):
        ...
        plt.title(f'Positive Distance Distribution')
        ...
        plt.title(f'Negative Distance Distribution')
        plt.xlabel('Distance to TSS (bp)')
        plt.ylabel('Count')
        plt.tight_layout()
