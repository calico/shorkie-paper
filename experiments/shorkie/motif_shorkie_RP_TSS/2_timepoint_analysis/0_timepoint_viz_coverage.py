...
def main():
    ...
    if timepoint_coverage:
        ...
        # Sort timepoints numerically using the numeric part extracted from the time string.
    ...
    # extend 50 bp upstream and downstream, then visualize the coverage for 
    # each full region across timepoints.
    ###########################################################################
    if options.bed and options.gtf:
        ...
        for idx_gtf, row in gene_gtf_df.iterrows():
            ...
            if gene_name:
                # If a gene appears more than once, take the minimal start and maximal end.
        ...
        # and then compute & plot the average bigwig coverage from each timepoint.
        for idx, bed_row in bed_df.iterrows():
            ...
            # Use the basename of the experiment directory, the BED order index, gene name, chromosome, and coordinate.
            exp_basename = os.path.basename(os.path.normpath(options.exp_dir))
            gene_dir = os.path.join(options.out_dir, options.exp_type, exp_basename, f"{idx:03d}_{gene}_{region_chrom}_{extended_start}-{extended_end}")
            os.makedirs(gene_dir, exist_ok=True)
            x = np.arange(extended_start, extended_end)
            # Combined plot across timepoints for this gene.
            plt.figure(figsize=FIG_SIZE)
            for tp in sorted(gene_timepoint_cov.keys(), key=lambda x: int(x.replace('T', ''))):
                avg_cov = gene_timepoint_cov[tp]
                plt.plot(x, avg_cov, label=f"Timepoint {tp}", color=timepoint_colors[tp])
            plt.xlabel(f"Genomic Position ({region_chrom})")
            plt.ylabel("Normalized Coverage (RPM)")
            plt.title(f"Coverage for gene {gene}\nRegion: {region_chrom}:{extended_start}-{extended_end}")
            plt.legend()
            combined_filename = os.path.join(gene_dir, f"{gene}_combined_coverage.png")
            plt.tight_layout()
            plt.savefig(combined_filename)
            plt.close()
            print(f"Combined plot for gene {gene} saved to {combined_filename}")
            # Individual plots for each timepoint.
            for tp, avg_cov in gene_timepoint_cov.items():
                plt.figure(figsize=FIG_SIZE)
                plt.plot(x, avg_cov, label=f"Timepoint {tp}", color=timepoint_colors[tp])
                plt.xlabel(f"Genomic Position ({region_chrom})")
                plt.ylabel("Normalized Coverage (RPM)")
                plt.title(f"Coverage for gene {gene} ({tp})\nRegion: {region_chrom}:{extended_start}-{extended_end}")
                plt.legend()
                tp_filename = os.path.join(gene_dir, f"{gene}_coverage_{tp}.png")
                plt.tight_layout()
                plt.savefig(tp_filename)
                plt.close()
                print(f"Plot for gene {gene} timepoint {tp} saved to {tp_filename}")
