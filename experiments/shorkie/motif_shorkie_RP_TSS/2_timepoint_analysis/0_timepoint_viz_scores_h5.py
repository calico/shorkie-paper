...
###########################################################################
# Main routine to load the target file, add timepoint info, filter by target TF,
# and visualize DNA logos for each averaged time point based on the selected tracks.
###########################################################################
def main():
    ...
    # Add a 'timepoint' column by extracting the time information from the 'description' column.
    # This regex looks for patterns like 'T0', 'T5', 'T10', etc.
    ...
    with h5py.File(f"{options.exp_dir}/scores.h5", 'r') as h5_file:
        ...
        for score in scores:
            ...
            for tp, group_df in target_df.groupby('timepoint'):
                ...
                # Verify that the adjusted indices fall within the expected range.
                if max(adjusted_track_indices) >= N_tracks or min(adjusted_track_indices) < 0:
                    raise ValueError("One or more adjusted track indices are out-of-range.")

                # Create an output directory for the averaged DNA logos for this timepoint.
                track_dir = f"{options.exp_dir}/dna_logo_example/{options.target_tf}logos/{score}/{tp}_average"
                os.makedirs(track_dir, exist_ok=True)

                # Prepare lists to later stack logos for all sequences (optional stacking).
                ...
                for seq_idx in range(N_sequences):
                    ...
                    print(f"[score={score}, {tp} average] seq_idx={seq_idx} --> {chr_16k}:{start_16k}-{end_16k}")

                    # Get the one-hot representation of the reference sequence using your helper function
                    ...
                    # Get PWM data averaged over the selected tracks for the current timepoint.
                    pwm_data = compute_time_average_for_one_time(
                        dataset,
                        seq_idx=seq_idx,
                        track_indices=adjusted_track_indices
                    )
                    ...
                    # Plot and save the DNA logo for the current sequence and timepoint.
                    visualize_input_ism(
                        viz_pwm,
                    ...
                    print(f"Saved DNA logo for {tp} average, sequence {seq_idx}")

                # After processing all sequences for this timepoint, stack and save NPZ files.
                ...
                print(f"Saved concatenated tensors for {tp} average at:")
