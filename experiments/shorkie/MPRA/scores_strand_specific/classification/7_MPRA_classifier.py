...
def plot_trend_with_individuals(metrics_list, metric, title, output_file):
    ...
    ax.set_title(title)
    ax.grid(True)
    # Fix the panel and legend widths by adjusting the subplot
...
def plot_merged_trend_subplots(combined_quantiles, metric, title, output_file):
    """
    For combined genes, create one plot with three subplots (one for each quantile group: 5-25, 25-75, 75-95).
    In each subplot, the individual gene trends (light dashed lines) and the aggregated (mean ± std) trend 
    are plotted vs. insertion position. Each subplot includes its own legend.
    """
    quant_order = ["5-25", "25-75", "75-95"]
    # Adjust figure size as needed; here we use a 3-row layout
    ...
    for i, quant in enumerate(quant_order):
        ...
        if not group_list:
            ...
            continue
        # Assume all genes share the same insertion positions
        positions = sorted(group_list[0]["metrics"].keys())
        # Plot individual gene trends with legend labels
        ...
        # Compute aggregated trend (mean ± std)
        agg_means = []
        agg_stds = []
        for pos in positions:
    ...
    # Reduce the space between the suptitle and the first subplot:
...
def plot_group_roc_pr(metrics_list, group_label, output_dir):
    ...
    # Common grids for ROC/PR interpolation
    grid_fpr = np.linspace(0, 1, 100)
    grid_recall = np.linspace(0, 1, 100)
    
    # Number of panels and subplot arrangement
    n_panels = len(insertion_positions)
    ncols = min(3, n_panels)
    nrows = int(math.ceil(n_panels / ncols))

    # Prepare a colormap to assign a distinct color for each gene
...
def main():
    ...
    # --- Merged Trend Plots for Combined Genes as Subplots ---
    combined_quantiles = {}
    ...
    if combined_quantiles:
        plot_merged_trend_subplots(combined_quantiles, "auroc", 
            "AUROC trend for three gene expression quantiles", 
            os.path.join(trend_dir, "combined_auroc_trend_merged_subplot.png"))
        plot_merged_trend_subplots(combined_quantiles, "auprc", 
            "AUPRC trend for three gene expression quantiles", 
            os.path.join(trend_dir, "combined_auprc_trend_merged_subplot.png"))
