...
# GROUND_TRUTH_EXP = np.log2(GROUND_TRUTH_EXP + 0.01)  # Apply log2 transformation with epsilon to avoid log(0)
...
# ref_filename = os.path.join(root_dir, 'sample_submission.txt')
...
# REFERENCE_EXP = np.log2(REFERENCE_EXP + 0.01)  # Apply log2 transformation with 
######################################
...
def plot_scatter_with_regression(gt, pred, title, filename, corr_pearson, corr_spearman, label_name):
    ...
    # plt.scatter(gt, pred, color='green', alpha=0.5, label=label_name)
