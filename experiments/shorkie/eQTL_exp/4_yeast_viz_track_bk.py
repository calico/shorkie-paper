...
def load_scores_multifold(score, num_folds, root_dir):
    ...
    for fold in folds:
        snp_dir = os.path.join(root_dir, 'experiments', 'eQTL_exp', 'eqtls', f'eqtl_{fold}')
...
def plot_cov_plot(search_gene, gene, gene_start, gene_end, center_pos, chrom, poses, alts, seq_len,
                  seqnn_model, models, fasta_open, condition, targets_df, logSED_avg, snpweight):
    ...
    save_dir = os.path.join('./viz_tracks', condition)
    ...
    plot_coverage_track_pair_bins_w_ref(
        y_wt,
        y_mut,
        chrom,
        start,
