...
def plot_coverage_track_pair_bins_w_ref(
    ...
    # --- OVERRIDE to only show bin_window bins around center_bin ---
    half_win = bin_window // 2
    plot_start_bin = center_bin - half_win
    plot_end_bin   = center_bin + half_win
...
def plot_coverage_track_pair_bins_w_ref_zoomin(
    y_wt,
    y_mut,
    chrom,
    start,
    ...
    """
    Plot reference vs. alternate tracks alongside optional ground truth coverage,
    displaying only `bin_window` bins centered on the locus, and placing the
    legends outside the right side of the figure.

    Parameters
    ----------
    y_wt, y_mut : np.ndarray
        Wild-type and mutant model outputs, shape (1,1,n_bins,n_tracks).
    chrom : str
        Chromosome name.
    start : int
        Genomic coordinate of first bin start.
    center_pos : int
        Genomic coordinate of the focal SNP.
    pad : int
        Number of bins of padding around the unpadded output.
    bin_size : int
        Width of each bin in base pairs.
    bin_window : int
        Total number of bins to display (centered).
    (other params as before)
    """
    # --- determine plotting window in genomic coords ---
    plot_start = center_pos - plot_window // 2
    plot_end   = center_pos + plot_window // 2

    # --- compute full-window bin indices (unused after override) ---
    full_start_bin = (plot_start - start) // bin_size - pad
    full_end_bin   = (plot_end   - start) // bin_size - pad

    # --- compute center bin ---
    center_bin = (center_pos - start) // bin_size - pad

    # --- override to show only bin_window bins around center ---
    half_win = bin_window // 2
    plot_start_bin = center_bin - half_win
    plot_end_bin   = center_bin + half_win

    # --- handle transforms & clipping ---
    # convert to float32, undo scaling, clipping, power transforms
    def _prepare(arr, scale, transform, clip_soft):
        a = arr.astype(np.float32) / scale
        if clip_soft is not None:
            unclipped = (a - clip_soft)**2 + clip_soft
            mask = a > clip_soft
            a[mask] = unclipped[mask]
        return a ** (1.0 / transform)

    y_wt_curr = _prepare(y_wt, track_scales[0], track_transforms[0], clip_softs[0])
    y_mut_curr = _prepare(y_mut, track_scales[0], track_transforms[0], clip_softs[0])
    if y_ground_truth is not None:
        y_gt_curr = _prepare(y_ground_truth, track_scales[0], track_transforms[0], clip_softs[0])

    # --- average over channels & remove extra dims ---
    y_wt_curr = np.mean(y_wt_curr[..., track_indices[0]], axis=(0,1,3))
    y_mut_curr = np.mean(y_mut_curr[..., track_indices[0]], axis=(0,1,3))
    if y_ground_truth is not None:
        y_gt_curr = np.mean(y_gt_curr[..., ref_track_indices[0]], axis=(0,1,3))

    # --- optional normalization across window ---
    if normalize_counts:
        norm_slice = slice((normalize_window//2)//bin_size - pad,
                           -(normalize_window//2)//bin_size + pad or None)
        wt_sum = np.sum(y_wt_curr[norm_slice])
        y_wt_curr *= wt_sum / (np.sum(y_wt_curr[norm_slice]) + 1e-8)
        y_mut_curr *= wt_sum / (np.sum(y_mut_curr[norm_slice]) + 1e-8)
        if y_ground_truth is not None:
            y_gt_curr *= wt_sum / (np.sum(y_gt_curr[norm_slice]) + 1e-8)
    ...
    # --- apply log/sqrt scaling if requested ---
    if log_scale:
        y_wt_curr = np.log2(y_wt_curr + 1)
        y_mut_curr = np.log2(y_mut_curr + 1)
        if y_ground_truth is not None:
            y_gt_curr = np.log2(y_gt_curr + 1)
    elif sqrt_scale:
        y_wt_curr = np.sqrt(y_wt_curr + 1)
        y_mut_curr = np.sqrt(y_mut_curr + 1)
        if y_ground_truth is not None:
            y_gt_curr = np.sqrt(y_gt_curr + 1)

    # --- set plotting limits ---
    max_y = max(np.max(y_wt_curr), np.max(y_mut_curr),
                np.max(y_gt_curr) if y_ground_truth is not None else 0)
    ...
    # make room on right for legend
    f.subplots_adjust(right=0.75)

    # --- plot WT and Mutant bars ---
    x = np.arange(plot_start_bin, plot_end_bin)
    ax1.bar(x, y_wt_curr, width=1, alpha=0.7, label="Ref")
    if plot_mut:
        ax1.bar(x, y_mut_curr, width=1, alpha=0.7, label="Alt")

    # --- optional ground truth ---
    if ax2 is not None:
        ax2.bar(x, y_gt_curr, width=1, alpha=0.8, color="mediumseagreen", label="Ground Truth")

    # --- add vertical lines: center, gene boundaries, SNP ---
    ax1.vlines(center_bin, 0, max_y, linestyle="--", color="k", label="Center")
    ax1.vlines((gene_start - start)//bin_size - pad,
               0, max_y, linestyle="--", color="r", label=f"{search_gene} Start")
    ax1.vlines((gene_end - start)//bin_size - pad,
               0, max_y, linestyle="--", color="r", label=f"{search_gene} End")
    mut_bin = (poses[0] - start)//bin_size - pad
    ax1.scatter([mut_bin], [0.05*max_y], marker="*", s=100, color="k", label="SNP")

    # --- legend outside on the right ---
    ax1.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=12, frameon=False)
    if ax2 is not None:
        ax2.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=12, frameon=False)

    # --- axis labels & title ---
    ax1.set_ylabel("Signal", fontsize=12)
    if ax2 is not None:
        ax2.set_ylabel("GT Signal", fontsize=12)
        ax2.set_xlabel(f"{chrom}:{plot_start}-{plot_end} ({plot_window}bp)", fontsize=12)
    else:
        ax1.set_xlabel(f"{chrom}:{plot_start}-{plot_end} ({plot_window}bp)", fontsize=12)

    plt.tight_layout()

    # --- save if requested ---
    if save_figs:
        out_png = os.path.join(save_dir,
                               f"yeast_{save_suffix}_track_{track_indices[0][0]}_{track_indices[0][-1]}.png")
