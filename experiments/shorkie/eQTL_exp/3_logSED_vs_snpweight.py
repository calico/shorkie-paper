...
def main():
    ...
    all_data = df_eqtl[['Position_Gene', 'SnpWeight']].copy()
    ...
    plot_data = all_data.dropna(subset=['SnpWeight', f'{score}_mean']).copy()
    
    if len(plot_data) > 1:
        avg_corr_val, avg_p_val = stats.pearsonr(plot_data['SnpWeight'], plot_data[f'{score}_mean'])
    else:
