...
# TSV_DIR      = os.path.join(ROOT, 'experiments', 'SUM_data_process', 'eQTL_exp','results','viz')
SEED         = 42
...
for neg_set in range(1,5):
    ...
    TSV_DIR      = os.path.join(ROOT, 'experiments', 'SUM_data_process', 'eQTL_exp','results',f'set{neg_set}')
    ...
    for mdl in MPRA_MODELS:
        base   = os.path.join(ROOT, 'experiments', 'SUM_data_process', 'eQTL_MPRA_models_eval','results', mdl)
    ...
    pos_df = yeast_scores[yeast_scores['label'] == 1]
    neg_df = yeast_scores[yeast_scores['label'] == 0]
    if len(pos_df) > len(neg_df):
        pos_df = pos_df.sample(len(neg_df), random_state=SEED)
    elif len(neg_df) > len(pos_df):
        neg_df = neg_df.sample(len(pos_df), random_state=SEED)
    yeast_scores = pd.concat([pos_df, neg_df], ignore_index=True)
    print(f"[Yeast] balanced to {len(pos_df)} pos / {len(neg_df)} neg")
    ...
    # for mdl, df in mpra_balanced.items():
    #     prec_m, rec_m, _ = precision_recall_curve(df['label'], df['score'])
    #     ap_m             = average_precision_score(df['label'], df['score'])
    #     plt.plot(rec_m, prec_m,
    #             linestyle='-.', linewidth=2,
    #             label=f"{MPRA_NAME_MAP[mdl]} (AUPRC={ap_m:.2f})")
