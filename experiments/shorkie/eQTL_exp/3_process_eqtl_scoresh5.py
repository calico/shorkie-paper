...
def load_eqtl_data(eqtl_tsv):
    ...
    return df[['Position_Gene', 'EffectSize', 'Chr', 'ChrPos', 'Pheno']]
...
def load_scores(h5_path, score):
    ...
    with h5py.File(h5_path, 'r') as hdf:
        ...
        for pos in grp:
            for gene in grp[pos]:
                key = f"{pos}_{gene}"
                print(f"Processing {pos}; {gene}; {key}...")
                print(f"grp[pos][gene][:]: {grp[pos][gene][:].shape}")
...
def process_fold(eqtl_df, tss_data, base_dir, fold, score_name, is_positive, neg_subdir=None):
    ...
    if is_positive:
        ...
        df["Pheno"] = df['Position_Gene'].str.split('_').str[-1]
...
#         group_cols = ['Position_Gene', 'Chr', 'ChrPos', 'Pheno', 'label', 'label_type', 'EffectSize', 'distance']
#         df_avg = (
#             df_concat
...
def main():
    root      = '/home/kchao10/scr4_ssalzbe1/khchao/Yeast_ML'
    eqtl_tsv  = '/home/kchao10/scr4_ssalzbe1/khchao/Yeast_ML/data/eQTL/selected_eQTL/intersected_data_CIS.tsv'
    gtf       = os.path.join(root, 'data', 'eQTL', 'neg_eQTLS', 'GCA_000146045_2.59.gtf')
