...
def plot_cov_plot(search_gene, gene, gene_start, gene_end, center_pos, chrom, poses, alts, seq_len,
                  seqnn_model, models, fasta_open, condition, targets_df, logSED_avg, snpweight, bin_window=210):
...
def main():
    ...
    for idx, row in upregulated_df.iterrows():
        # print("process_eqtl_row: ", process_eqtl_row)
        if (row['Chr'] == "chrXIV" and row['ChrPos'] == 200328 and row['Pheno'] == "YNL239W") or \
            (row['Chr'] == "chrXI" and row['ChrPos'] == 604356 and row['Pheno'] == "YKR087C"):
            print("row:", row)
